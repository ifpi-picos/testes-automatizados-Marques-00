...
describe("Testando classe ContaBancaria", () => {
  ...
  test("metodo transferir", () => {
    ...
    //a expectativa é que eu transfira 100 pra segunda conta e fique com 100
  })
  ...
  test("metodo transferir teste de erro", () => {
    ...
    //a expectativa é que tente transferir 300 (mas é mais do que tem na conta), então não dá certo
  })

  test("metodo consultar saldo", () => {
    conta.depositar(200) //depositei 200
    expect(conta.consultarSaldo()).toBe(200)
    //a expectativa é que, ao consultar o saldo, realmente tenha 200
  })

  test("metodo exibir extrato", () => {
    conta.depositar(200)
    conta.sacar(100)
    expect(conta.exibirExtrato()).toBe("1. Depósito de R$ 200.00\n2. Saque de R$ 100.00")
  })

  test ("metodo registrar operacao",() => {
    expect(conta.registrar)
  })
